...
\begin{itemize}
    \item \textbf{Case A:} Some cop $C$ is at vertex $u$, $R$ is at $r$ and $R_i$ is  defined to be the vertices of the connected component containing $r$ in $G - u$ (Same situation as step 0).
    \item \textbf{Case B:} %Describe Case B here. Note from Mohamed: I assumed that R_i is a subset of V_G and not a subgraph because it made more sense. If you read the proof from the original paper, keep that in mind while applying it here.
\end{itemize}

We will now prove that the condition holds in case A.\\
Suppose $u$ has only one neighbour $v$ in $R_i$. Then, $C$ would move from $u$ to $v$. If $v = r$, then game was over. If not, no matter how $R$ moves next, $R_{i + 1} = R_i - \{v\}$. Hence, we are again back at case A with $R_{i + 1} \subset R_i$ as desired.\\
If $u$ has more than one neighbour in $R_i$, pick any two neighbours $a$ and $b$. Let $P$ be the shortest path between $a$ and $b$ in $R_i$. By Lemma 3.7, another cop $D$ controls $P$ after a finite number of moves. Thus, we reach case $B$ where cop $C$ controls $P_1 = (a, u, b)$ and cop $D$ controls $P_2 = P$ and $R_{i + 1} \subseteq R_i - V_P \subset R_i$. Thus, the condition still holds as desired.
    %case(a) --> AboElFotouh
     %case(b(i)) --> Bassel
     %case(b(i)) --> Mahmoud
