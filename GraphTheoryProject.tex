...
\textbf{Case(A)} with $u = a$ and $R_{i+1} = A$, so that $R_{i+1} \subsetneq R_i$ as required.
    ...
    \item \textbf{Case B(ii):} Suppose that the robber territory $R_i$ is adjacent to both $P_1$ and $P_2$. Let $x \in V(P_1)$ and $y \in V(P_2)$ be vertices that have neighbors inside $R_i$. We can define $Q$ as a shortest path between $x$ and $y$ such that all internal vertices of $Q$ lie strictly within the robber's territory $R_i$.
    
    By Lemma \ref{lem:iso} the free cop $C_3$ can move to control the path $Q$ in a finite number of moves, and the robber cannot cross $Q$ without being captured. Because the graph $G$ is planar, the union of the path segment from $u$ to $x$ on $P_1$, the path segment from $u$ to $y$ on $P_2$, and the new path $Q$ forms a closed cycle that divides the plane. The robber is therefore forced into a new territory $R_{i+1}$ which is strictly contained within $R_i$ (since $Q$ cuts through $R_i$).
    
    Crucially, the boundary of this new, smaller territory $R_{i+1}$ is now formed by $Q$ and at most one of the original paths (or a combination that forms two new isometric paths). Because the new boundary requires at most two cops to control (for example, $C_3$ on $Q$ and $C_1$ on $P_1$), the remaining cop ($C_2$) is freed. We have successfully reduced the robber's territory so that $R_{i+1} \subset R_i$ while maintaining a free cop.
\end{itemize}
